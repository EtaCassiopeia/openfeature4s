...
final private[zio] class FeatureFlagsLive(
  client: OFClient,
  ...
  private[zio] def startEventBridge: ZIO[Scope, Nothing, Unit] = {
      ...
      case Some(overrideValue) =>
        ...
        flagType.decode(overrideValue) match {
          case Right(decoded) =>
            ...
            FlagEvaluationOps.overridden(key, decoded).flatMap { eval =>
  ...
  private def evaluateAndCache[A: FlagType](
    ...
    for {
      ...
      eval       <- FlagEvaluationOps.evaluated(key, resolution)
      ...
      ClientEvaluator.evaluateStandard[A](flagType.typeName, client, key, default, ofContext) match {
        case Some(erased) =>
          val timedEval = timeout match {
            case Some(d) =>
              ZIO
                .attemptBlocking(erased.call())
                .disconnect
                .timeoutFail(new java.util.concurrent.TimeoutException(s"Evaluation of '$key' timed out after $d"))(d)
            case None => ZIO.attemptBlocking(erased.call())
          }
          timedEval
