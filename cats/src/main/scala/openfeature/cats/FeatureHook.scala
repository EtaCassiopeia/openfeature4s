...
trait FeatureHook[F[_]] {
  ...
  /** Called before flag evaluation. May return an updated context and hints.
    ...
    */
  def before(ctx: HookContext, hints: HookHints)(implicit
    F: Applicative[F]
  ): F[Option[(EvaluationContext, HookHints)]] =
    F.pure(None)

  /** Called after a successful flag evaluation. */
  def after[A](ctx: HookContext, details: FlagResolution[A], hints: HookHints)(implicit F: Applicative[F]): F[Unit] =
    F.unit

  /** Called when flag evaluation fails. */
  def error(ctx: HookContext, err: FeatureFlagError, hints: HookHints)(implicit F: Applicative[F]): F[Unit] =
    F.unit

  /** Called unconditionally after evaluation (success or failure). */
  def finallyAfter(ctx: HookContext, details: Option[FlagResolution[_]], hints: HookHints)(implicit
    F: Applicative[F]
  ): F[Unit] =
    F.unit
}
