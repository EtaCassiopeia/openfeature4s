...
class HookSpec extends CatsEffectSuite {
  ...
  private def recordingHook(log: Ref[IO, List[String]]): FeatureHook[IO] = new FeatureHook[IO] {
    override def before(ctx: HookContext, hints: HookHints)(implicit
      F: Applicative[IO]
    ): IO[Option[(EvaluationContext, HookHints)]] =
      log.update(_ :+ "before").as(None)
    override def after[A](ctx: HookContext, details: FlagResolution[A], hints: HookHints)(implicit
      F: Applicative[IO]
    ): IO[Unit] =
      log.update(_ :+ "after")
    override def finallyAfter(ctx: HookContext, details: Option[FlagResolution[_]], hints: HookHints)(implicit
      F: Applicative[IO]
    ): IO[Unit] =
  ...
  provider.test("custom hook before can modify context") { _ =>
    ...
    hook.before(ctx, HookHints.empty).map {
