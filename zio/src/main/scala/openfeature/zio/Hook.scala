...
object FeatureHook {
  ...
  def compose(hooks: List[FeatureHook]): FeatureHook = new FeatureHook {
    ...
    private lazy val hookDataMap: Map[FeatureHook, openfeature.model.HookData] =
      hooks.map(h => h -> openfeature.model.HookData.empty).toMap

    private def ctxForHook(ctx: HookContext, hook: FeatureHook): HookContext =
      ctx.copy(hookData = hookDataMap.getOrElse(hook, openfeature.model.HookData.empty))
