...
import openfeature.model.{EvaluationContext, FlagEvaluation, FlagResolution, TransactionResult}
import zio._
import zio.test._
  ...
  def spec = suite("TransactionSpec")(
    suite("FlagEvaluation")(
      test("evaluated creates record with wasOverridden = false") {
        ...
        for {
          eval <- FlagEvaluationOps.evaluated("test-flag", resolution)
      ...
      test("overridden creates record with wasOverridden = true") {
        for {
          eval <- FlagEvaluationOps.overridden("override-flag", 42)
      ...
      test("evaluation has timestamp") {
        ...
        for {
          ...
          eval   <- FlagEvaluationOps.evaluated("test", resolution)
          after  <- Clock.instant
    ...
    suite("TransactionState")(
      ...
      test("record adds evaluation") {
        for {
          state <- TransactionState.make(Map.empty, EvaluationContext.empty)
          eval  <- FlagEvaluationOps.evaluated("test", FlagResolution.default("test", true))
          _     <- state.record(eval)
          evals <- state.getEvaluations
      ...
      test("toResult builds TransactionResult") {
        for {
          ...
          eval1  <- FlagEvaluationOps.evaluated("provider", FlagResolution.default("provider", true))
          eval2  <- FlagEvaluationOps.overridden("override", 42)
      ...
      test("getCachedEvaluation returns evaluation after record") {
        for {
          state  <- TransactionState.make(Map.empty, EvaluationContext.empty)
          eval   <- FlagEvaluationOps.evaluated("cached-flag", FlagResolution.default("cached-flag", "cached-value"))
          _      <- state.record(eval)
